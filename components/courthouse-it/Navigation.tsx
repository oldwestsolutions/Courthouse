...
import { itNavItems } from "@/lib/courthouse-it-data";
  ...
  return (
    ...
    >
      <nav className="ch-shell flex h-[60px] items-center justify-between md:h-[72px]">
        <a href="#top" className="flex items-center" aria-label="courthouse.it.com home">
          <Wordmark />
        </a>
        ...
        <div className="hidden items-center gap-8 md:flex">
          ...
          <div className="flex items-center gap-3">
            <GhostBtn>Employee Login</GhostBtn>
            <FilledBtn>Submit Ticket</FilledBtn>
          </div>
        </div>
          ...
          >
            <div className="ch-shell flex flex-col gap-1 py-5">
              ...
              <div className="mt-4 flex flex-col gap-3">
                <GhostBtn full>Employee Login</GhostBtn>
                <FilledBtn full>Submit Ticket</FilledBtn>
              </div>
            </div>
          </motion.div>
        )}
...
function GhostBtn({ children, full }: { children: React.ReactNode; full?: boolean }) {
  return (
    <button
      type="button"
      className={`min-h-[44px] rounded-[3px] text-[12px] font-normal uppercase transition-colors duration-200 ${full ? "w-full" : ""}`}
      style={{
...
function FilledBtn({ children, full }: { children: React.ReactNode; full?: boolean }) {
  return (
    <button
      type="button"
      className={`min-h-[44px] rounded-[3px] text-[12px] font-semibold uppercase transition-colors duration-200 ${full ? "w-full" : ""}`}
      style={{
        letterSpacing: "0.08em",
        backgroundColor: "var(--ch-gold)",
        color: "var(--ch-navy)",
